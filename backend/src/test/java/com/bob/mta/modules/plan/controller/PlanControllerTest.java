...
class PlanControllerTest {
    ...
    void analyticsShouldSummarizePlans() {
        var analytics = controller.analytics(null, null, null, null, null, null, null, null, null, null).getData();
    ...
    @Test
    void analyticsShouldRespectCustomLimitsAndDueSoonWindow() {
        OffsetDateTime base = OffsetDateTime.now();
        CreatePlanCommand firstUpcoming = new CreatePlanCommand(
                "tenant-limit",
                "Limit upcoming A",
                "Upcoming plan for owner A",
                "cust-limit",
                "limit-owner-a",
                base.plusHours(1),
                base.plusHours(2),
                "Asia/Tokyo",
                List.of("limit-owner-a"),
                List.of(new PlanNodeCommand(null, "检查准备", "CHECKLIST", "limit-owner-a", 1, 30,
                        PlanNodeActionType.NONE, 100, null, "", List.of()))
        );
        var planA = planService.createPlan(firstUpcoming);
        planService.publishPlan(planA.getId(), "limit-owner-a");

        CreatePlanCommand secondUpcoming = new CreatePlanCommand(
                "tenant-limit",
                "Limit upcoming B",
                "Upcoming plan for owner B",
                "cust-limit",
                "limit-owner-b",
                base.plusHours(2),
                base.plusHours(3),
                "Asia/Tokyo",
                List.of("limit-owner-b"),
                List.of(new PlanNodeCommand(null, "检查执行", "CHECKLIST", "limit-owner-b", 1, 30,
                        PlanNodeActionType.NONE, 100, null, "", List.of()))
        );
        var planB = planService.createPlan(secondUpcoming);
        planService.publishPlan(planB.getId(), "limit-owner-b");
        ...
        CreatePlanCommand dueSoonCommand = new CreatePlanCommand(
                ...
                "Asia/Tokyo",
                List.of("limit-due"),
                List.of(new PlanNodeCommand(null, "风险检查", "CHECKLIST", "limit-due", 1, 15,
                        PlanNodeActionType.NONE, 100, null, "", List.of()))
        );
        var dueSoonPlan = planService.createPlan(dueSoonCommand);
        planService.publishPlan(dueSoonPlan.getId(), "limit-due");

        CreatePlanCommand laterRisk = new CreatePlanCommand(
                "tenant-limit",
                "Future maintenance",
                "Risk outside window",
                "cust-limit",
                ...
                "Asia/Tokyo",
                List.of("limit-far"),
                List.of(new PlanNodeCommand(null, "后续检查", "CHECKLIST", "limit-far", 1, 20,
                        PlanNodeActionType.NONE, 100, null, "", List.of()))
        );
        var farPlan = planService.createPlan(laterRisk);
        planService.publishPlan(farPlan.getId(), "limit-far");

        var analytics = controller.analytics("tenant-limit", null, null, null, null,
                        base, 1, 1, 1, 30)
                .getData();

        assertThat(analytics.getUpcomingPlans()).hasSize(1);
        assertThat(analytics.getOwnerLoads()).hasSize(1);
        assertThat(analytics.getRiskPlans()).hasSize(1);
        assertThat(analytics.getRiskPlans())
                .allSatisfy(risk -> {
                    assertThat(risk.getRiskLevel()).isEqualTo(PlanAnalytics.RiskLevel.DUE_SOON);
                    assertThat(risk.getMinutesUntilDue()).isLessThanOrEqualTo(30);
                });
    }
