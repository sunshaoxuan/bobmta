...
- **当前进展**：
  ...
  - 将计划多视图 Segmented 切换集成进 `PlanListBoard`，复用客户聚合与日历事件派生数据驱动客户/日历视图，并新增 Node Test 覆盖聚合结果，保证筛选与分页在视图切换时保持一致。
  - 重构应用头部为 Layout.Header + Menu 导航，结合会话角色拉取或 Mock 菜单并在权限不足时自动重定向，同时补充用户信息下拉、登出入口与菜单降级提示；新增未登录态的「登录」快捷入口，并在菜单接口返回 403 时于内容区弹出权限不足告警。
