...
- ✅ 依据计划状态切换详情设计/执行模式，执行态高亮当前节点并锁定已完成节点、提示切换回设计态；PlanPreview 重构为基于模式枚举的配置表，统一渲染节点树/操作区标题与控件开关，设计态持续开放节点编辑入口且提醒编辑按钮仅在设计态渲染，执行态在已完成/取消计划中自动收起节点操作；同步拆分 `DesignModePanel`、`ExecutionModePanel` 子组件承载提示与辅助信息，状态容器暴露模式与当前节点上下文，并以新增单元测试覆盖取消态、设计↔执行切换、回退草稿与嵌套节点执行场景。
