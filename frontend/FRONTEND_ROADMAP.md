...
- **当前进展**：
  ...
  - ✅ 依据计划状态切换详情设计/执行模式，执行态高亮当前节点并同步显示进度，设计态提供节点编辑入口并在模式切换时禁用编辑操作；模式提示面板拆分为可复用组件嵌入详情分区，并向子组件传递模式标记，状态容器暴露模式与当前节点信息并以新增单元测试覆盖取消态与设计→执行模式切换逻辑。
  ...
  - ✅ 升级计划列表多视图为 Tabs + Segmented 联动，新增客户 List/Tree 视图与 Calendar 日历视图，复用派生聚合与事件映射并通过 Node Test 验证按客户/日期分组结果。
