...
- **当前进展**：
  ...
  - ✅ 依据计划状态切换详情设计/执行模式，执行态高亮当前节点并同步显示进度，设计态提供节点编辑入口并在模式切换时禁用编辑操作；模式提示面板拆分为可复用组件嵌入详情分区，并向子组件传递模式标记，状态容器暴露模式与当前节点信息并以新增单元测试覆盖取消态切换逻辑。
  ...
  - ✅ 抽离 `HeaderNav` 组件承载品牌、权限菜单与用户区，Session 容器统一获取后端或 Mock 导航配置并按角色裁剪；未登录仅显示「登录」按钮隐藏菜单，鉴权成功后保留用户信息/登出入口并在接口失败时降级为 Mock 菜单提示网络状态，同时将导航固定在页面顶部避免滚动遮挡内容。
  - ✅ 交付客户分组与日历视图组件，基于计划列表派生客户聚合与时间桶事件，支持 Segmented 切换日/周/月/年粒度且保留筛选条件，并以 Node Test 校验聚合排序、时长计算及周起始边界。
