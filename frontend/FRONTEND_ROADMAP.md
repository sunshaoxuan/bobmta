...
- **当前进展**：
  ...
  - ✅ 依据计划状态切换详情设计/执行模式，执行态高亮当前节点并锁定已完成节点、提示切换回设计态；模式提示面板拆分为可复用组件嵌入详情分区，PlanPreview 依据 `PlanDetail.status` 映射模式并向节点树、提醒看板与操作面板传递标记以控制编辑，设计态持续开放节点编辑入口且提醒编辑按钮仅在设计态渲染；状态容器暴露模式与当前节点信息，并以新增单元测试覆盖取消态、设计↔执行切换、回退草稿与嵌套节点执行场景，同时进一步拆分模式提示面板、执行态弱化已完成节点并补充模式/活跃节点选择器与切换回退用例。
