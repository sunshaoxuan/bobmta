...
- ✅ 依据计划状态切换详情设计/执行模式，执行态高亮当前节点并锁定已完成节点、提示切换回设计态；PlanPreview 重构为基于模式枚举的配置表，统一渲染节点树/操作区标题与控件开关，设计态持续开放节点编辑入口且提醒编辑按钮仅在设计态渲染，执行态在已完成/取消计划中自动收起节点操作；同步拆分 `DesignModePanel`、`ExecutionModePanel` 子组件承载提示与辅助信息，状态容器暴露模式与当前节点上下文，并以新增单元测试覆盖取消态、设计↔执行切换、回退草稿与嵌套节点执行场景；最新一轮将提醒分区纳入同一模式配置表并引入模式化辅助提示，新增深层待办节点定位用例验证执行态当前节点推断逻辑。
  - ✅ 重构 `PlanPreview` 模式配置，抽象 `PLAN_PREVIEW_MODE_DEFINITIONS` 统筹设计/执行态的标题、提示与操作清单，提醒看板接受模式驱动的编辑开关并在执行态自动清除节点编辑状态；`DesignModePanel`、`ExecutionModePanel` 持续拆分展示模式提示，状态容器补充分支单测覆盖空节点与遗留字段差异，验证模式与当前节点选择器的一致性；提醒分区配置新增统一的标题/提示定义，helper 渲染也接入模式面板逻辑，确保所有详情分区在设计与执行模式下行为一致。
  ...
  - ✅ 重构 `HeaderNav` 组件承载品牌标识与固定导航，使用 `Layout.Header`、`Menu`、`Dropdown` 组织品牌、导航与用户菜单；Session 容器同步输出导航配置、角色权限与用户菜单数据源，组件内按角色过滤菜单项并在 401/403 时回退 Mock。401 会清理会话并提示重新登录，未登录保留访客徽标与登录按钮提示，403 时在导航区提示权限告警。
...
- 2025-09-29：交付全局 `HeaderNav` 导航组件，接入 Session 导航配置与角色过滤，导航栏固定吸顶并支持 401/403 降级提示。
