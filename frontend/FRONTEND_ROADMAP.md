...
- **当前进展**：
  ...
  - ✅ 重构 `PlanPreview` 模式配置，抽象 `PLAN_PREVIEW_MODE_DEFINITIONS` 统筹设计/执行态的标题、提示与操作清单，并以状态枚举限定执行动作的展示条件；提醒看板接受模式驱动的编辑开关并在执行态自动清除节点编辑状态，同时在只读模式下隐藏提醒启停按钮；`DesignModePanel`、`ExecutionModePanel` 拆分展示模式提示并支持自定义描述/标签，状态容器补充分支单测覆盖空节点与遗留字段差异，新增计划状态选择器验证模式/节点上下文一致性；提醒分区配置新增统一的标题/提示定义，helper 渲染也接入模式面板逻辑，确保所有详情分区在设计与执行模式下行为一致。
