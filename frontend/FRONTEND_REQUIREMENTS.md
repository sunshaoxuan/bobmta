...
| F-004 | 计划统计驾驶舱 | `GET /api/v1/plans/analytics` | 按状态、负责人、逾期风险等聚合数据 | ✅ 完成 | 🟡 规划中 | 参考阶段三文档构造统计 Mock | 已支持 `ownerId`、`referenceTime`、`upcomingLimit`、`ownerLimit`、`riskLimit`、`dueSoonMinutes` 查询参数，详见《docs/backend-requests/plan-analytics-dashboard.md》，便于前端按卡片容量与提醒窗口定制驾驶舱 |
