...
| F-002 | 计划详情与时间线视图 | `GET /api/v1/plans/{id}`、`GET /api/v1/plans/{id}/timeline` | 计划节点、附件、时间线事件的字段需确认必填项 | 🟡 规划中 | 🛠️ 开发中 | 已补充计划详情/时间线/提醒 Mock 样例与缓存策略单测，页面以占位组件展示数据结构 | 需要后端补充时间线事件类型说明，当前前端已上线节点树视图与字段归一化作为占位，并完成时间线筛选快照、缓存联动与 URL 查询参数同步治理 |
