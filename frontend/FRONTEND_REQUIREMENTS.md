...
| F-006 | 计划多视图驾驶舱 | `GET /api/v1/plans` 提供客户信息、计划时间窗 | 需要返回客户标识/名称及计划窗口（开始/结束）字段 | 🟡 规划中 | ✅ 完成（迭代 #2 视图扩展） | 扩展 `listMockPlans` 与 `planDetail` 样例补充客户字段，生成客户聚合与日历事件并以 Node Test 校验聚合准确性 | 将 Segmented 切换集成到 `PlanListBoard`，复用派生数据驱动客户分组与日历视图，URL 同步保留筛选参数，等待后端补齐真实数据 |
