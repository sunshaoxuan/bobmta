...
| F-002 | 计划详情与时间线视图 | `GET /api/v1/plans/{id}`、`GET /api/v1/plans/{id}/timeline` | 计划节点、附件、时间线事件的字段需确认必填项 | 🟡 规划中 | 🛠️ 开发中 | 复用计划列表缓存渲染预览卡片，待补充详情/时间线 JSON 样例驱动单元测试 | 需要后端补充时间线事件类型说明 |
