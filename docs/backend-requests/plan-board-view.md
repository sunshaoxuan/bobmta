...
即便底层服务返回 `null` 视图，也会默认使用 `WEEK` 作为粒度：
...
- 新增后端单元测试 `PlanBoardResponseTest`，验证 DTO 转换在缺失视图时统一回退零指标，构造函数可折叠空集合并回退粒度，
  并确保客户分组、时间桶与计划卡片字段与 `PlanBoardView` 完整对齐。
...
- 新增 `PlanControllerTest.boardShouldOrderCustomerGroupsByTotals` 验证客户分组按计划总数降序、同总数时按 `customerId` 升序排列，同时确认时间桶按开始时间排序，保持前端多视图切换的顺序一致性。
