...
- 扩展前端计划列表的筛选、详情导航与提醒配置操作，沉淀认证态缓存与接口契约的端到端
...
### 🔄 正在进行
- 迭代 #1：整理前后端协同规范，建立需求清单与迭代路线图，梳理计划总览视图后续的状态管理与错误处理需求。
...
### 🔄 正在进行
- 迭代 #1：整理前后端协同规范，建立需求清单与迭代路线图，梳理计划总览视图后续的状态管理与错误处理需求。
  - 📌 离线环境基线：完成前端依赖本地化与编译/预览脚本，`npm install`、`npm run build` 在无外网环境下均可执行，并新增开发指引文档。
...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - 📌 下一步：补充 Mock 层与单元测试基线，覆盖接口异常分支，并抽象请求重试与 Loading/Empty/Error 组件。
...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - 📌 下一步：抽象可复用的 Loading/Empty/Error 呈现组件，并为计划列表筛选交互准备 UI 骨架与状态管理扩展。
