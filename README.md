...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - ✅ 已完成：依据计划状态区分设计/执行模式，执行态高亮当前节点并弱化已完成节点，设计态提供节点编辑入口并在模式切换时自动禁用编辑。
  - ✅ 已完成：抽离全局头部为 `HeaderNav` 组件，Session 状态同步后端或 Mock 导航配置并按角色过滤；未登录仅保留「登录」按钮并隐藏菜单，鉴权成功后仍提供用户信息下拉与登出入口，并在接口失败时降级至 Mock 菜单标记网络状态；导航栏固定于视口顶部防止滚动时遮挡内容。
  - ✅ 已完成：交付计划多视图 Segmented 切换，提供客户聚合卡片与日/周/月/年日历视图，沿用筛选条件并同步 URL 搜索参数以支撑 F-006 需求复用，并以 Node Test 校验客户聚合与时间桶排序、时长计算及跨周边界处理。
