...
- 迭代 #2（✅ 已完成）：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - ✅ 已完成：重构全局 `HeaderNav` 组件，使用 `Layout.Header` + `Menu` + `Dropdown` 承载品牌区、角色敏感导航与用户菜单，新增固定吸顶布局、401/403 告警提示与加载指示；Session 状态补充 `navigationSummary` 聚合导航配置、规范化角色与鉴权态标记，并新增 `loading`/`source` 元数据方便导航降级提示，同时继续暴露 `navigationPathMap`/`navigationPaths`/`canAccessPath` 等能力。组件内部按角色过滤菜单并在接口异常时降级为 Mock，保留后端 `children` 层级以便扩展子菜单，新增导航区域无障碍属性与空菜单回退布局。未登录场景仅保留访客徽标与登录入口，不再渲染导航项。
  - ✅ 已完成：为 `HeaderNav` 补齐离线依赖导入路径，统一通过 `vendor/antd` 与 `vendor/react` 复用预置组件包装层，避免直接引用外部包导致的构建失败，并保持固定导航样式。 
  ...
  - 📌 后续在迭代 #3 中继续跟进：等待后端提供节点执行与提醒更新接口后对接真实调用，并补齐操作失败提示与权限校验的前端展现。
...
### 🔄 正在进行
- 迭代 #3：扩展计划列表筛选与详情路由骨架，集中管理 `/` 与 `/plans/:planId` 页面跳转，并推进提醒策略视图的导航准备。
  - ✅ 首个里程碑：引入离线 React Router 封装 `<BrowserRouter>` 与 `AppRoutes`，统一路由声明并改写 URL 同步逻辑。
