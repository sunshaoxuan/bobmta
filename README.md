...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - ✅ 已完成：依据计划状态区分设计/执行模式，执行态高亮当前节点并弱化已完成节点，设计态提供节点编辑入口并在模式切换时自动禁用编辑；进一步拆分模式提示面板复用到详情分区，并在状态容器中暴露模式/当前节点信息且补充单元测试校验切换逻辑。
  - ✅ 已完成：升级全局头部导航，基于角色过滤一级菜单并在未授权访问时回退至首页，同时提供用户信息下拉与登出入口，后端接口缺失时回退 Mock 菜单并标记网络状态。
  - ✅ 已完成：将计划多视图 Segmented 切换内嵌至 `PlanListBoard`，复用客户聚合与日历事件派生数据驱动客户/日历子视图，并以 Node Test 校验聚合结果，确保筛选、分页与 URL 搜索参数在视图切换时保持一致。
