...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - ✅ 已完成：提炼计划详情模式上下文推导逻辑，`PlanPreview` 统一根据 `PlanDetail.status`、缓存上下文与节点树推导 `PlanViewMode` 与当前节点标记，并将模式透传至节点/提醒子组件；节点树补充 `data-editable` 标识与执行态禁用交互，提醒面板在只读模式下自动隐藏按钮；`planDetail` 状态选择器新增节点状态漂移用例，确保模式切换与节点定位回退时上下文一致。
  - ✅ 已完成：重构全局 `HeaderNav` 组件，使用 `Layout.Header` + `Menu` + `Dropdown` 承载品牌区、角色敏感导航与用户菜单，新增固定吸顶布局、401/403 告警提示与加载指示；Session 状态补充 `navigationSummary` 聚合导航配置、规范化角色与鉴权态标记，同时继续暴露 `navigationPathMap`/`navigationPaths`/`canAccessPath` 等能力，组件内部按角色过滤菜单并在接口异常时降级为 Mock，保留后端 `children` 层级以便扩展子菜单。未登录场景仅保留访客徽标与登录入口，不再渲染导航项。
