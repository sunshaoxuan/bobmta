...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - ✅ 已完成：依据计划状态区分设计/执行模式，执行态高亮当前节点并锁定已完成节点，模式面板提示切换回设计态；PlanPreview 重构为基于模式枚举的配置表，统一渲染节点/操作区标题与控件开关，设计态继续开放节点编辑入口且提醒编辑按钮仅在设计态可见，执行态在已完成/取消计划中收起节点操作；同步拆分 `DesignModePanel`、`ExecutionModePanel` 子组件包裹提示与辅助信息，状态容器暴露模式与当前节点上下文并新增单元测试覆盖取消态、设计↔执行切换、回退草稿与嵌套节点执行场景。
  - ✅ 已完成：重构全局 `HeaderNav` 组件，新增品牌标识与固定导航布局；Session 状态同时暴露角色权限与用户菜单数据源，导航栏在组件内按角色过滤并在接口异常时降级为 Mock；未登录场景以访客徽标 + 登录按钮提示，命中 403 时在导航栏展示权限告警并保留 Mock 菜单回退，401 会清理会话并在导航栏提示重新登录入口。
