...
- 迭代 #21：参数化计划驾驶舱统计输出，支持前端按照卡片容量与提醒窗口定制数据量。
  - `/api/v1/plans/analytics` 增加 `referenceTime`、`upcomingLimit`、`ownerLimit`、`riskLimit` 与 `dueSoonMinutes` 查询参数，可调整统计的时间基准、卡片容量及「即将到期」窗口。
  - 计划服务统一将定制参数传递给分析仓储，MyBatis 与内存实现均支持新的查询约束并复用默认值以兼容历史调用。
  - 控制层与服务层单元测试新增极值窗口、数量裁剪与风险过滤用例，验证定制参数生效后返回结构稳定。
