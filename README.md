...
- 迭代 #2：搭建状态管理与请求封装，梳理鉴权、计划列表与多语言的共享上下文，逐步替换页面内散落的网络请求逻辑。
  ...
  - ✅ 已完成：重构 `PlanPreview` 模式配置，抽象 `PLAN_PREVIEW_MODE_DEFINITIONS` 统一管理设计/执行态的标题、提示与操作清单，提醒面板接收模式驱动的编辑开关并在执行态自动清理节点编辑状态；`DesignModePanel`、`ExecutionModePanel` 继续分离呈现模式提示，状态容器补充空节点与遗留字段差异用例验证模式与当前节点选择器的稳健性。
